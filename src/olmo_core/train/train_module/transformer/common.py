    ...
    *,
    ...
    norm_modes = sum(
        ...
        for mode in (
            te_layer_norm,
            liger_layer_norm,
            liger_megatron_layer_norm,
            quack_layer_norm,
        )
    )
    if norm_modes > 1:
        raise OLMoConfigurationError(
            "Transformer Engine, Liger, Liger Megatron, and Quack RMSNorm modes are mutually exclusive."
        )
    ...
    if quack_layer_norm:
        ...
        for m in model_parts:
            for module in m.modules():
                if isinstance(module, RMSNorm):
                    module.enable_quack_rms_norm()
                    swapped += 1
        log.info("Enabled Quack RMSNorm kernels for %d norm module(s)", swapped)
